...
SoftwareSerial serial_to_device(10, 11); // RX, TX
LiquidCrystal lcd(8, 9, 4, 5, 6, 7); // select the pins used on the LCD panel
const u8 pin_power_for_sensor = 12;
...
void setup() {
    ...
    pinMode(pin_power_for_sensor, OUTPUT);
    digitalWrite(pin_power_for_sensor, true); // set +5V - will this be good enough for powering the sensor? I'm low on pins...
   
    
    lcd.begin(8, 2);              // start the library
    lcd.setCursor(0,0);
    lcd.print("Smogv0.1"); // print a simple message
...
void loop () {
  ...
  lcd.print(value_to_display);      // display seconds elapsed since power-up
  lcd.print("   ");
